...
void sendMavlinkStatusText(const char* text) {
  ...
  mavlink_msg_statustext_pack(
    MAV_SYS_ID,
    MAV_COMP_ID_SHARED_OUT,  // Changed to 156
    &msg,
    MAV_SEVERITY_INFO,
    text,
    0,
...
void fetchWeatherAndSendMavlink() {
  ...
  String locationStr = String(gps.lat, 5) + "," + String(gps.lon, 5);
  String url =
    "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/" + locationStr + "?unitGroup=metric"
                                                                                                           ...
                                                                                                           "sunriseEpoch,sunsetEpoch,moonphase,uvindex,solarradiation"
                                                                                                           "&include=current,days&key="
    + String(VISUALCROSSING_API_KEY) + "&contentType=json";
  ...
  snprintf(buf[2], 50, "WX 3/8: Rain:%.1fmm(%.0f%%) %s", precip, precipProb, pType);
  snprintf(buf[3], 50, "WX 4/8: Snow:%.1fmm Depth:%.1fmm", snow, snowDepth);
  snprintf(buf[4], 50, "WX 5/8: Wind:%.1f G:%.1f Dir:%d", windSpeed, windGust, windDir);
  ...
  Serial.println("[WX] Injecting 8-part weather sequence via MAVLink STATUSTEXT...");
  for (int i = 0; i < 8; i++) {
...
void fetchSpaceWeatherAndSendMavlink() {
  ...
  WiFiClientSecure client;
  client.setInsecure();  // Bypass explicit SSL certificate validation chain
  HTTPClient http;

  // GFZ Potsdam Nowcast URL
  String url = "https://kp.gfz.de/app/json/?start=2026-06-23T00:00:00Z&end=2026-06-24T23:59:59Z&index=Kp&status=nowcast";
  ...
  // Upgraded to ArduinoJson v7 unified memory allocation architecture
  JsonDocument doc;
  ...
  if (kpArray.size() > 0) {
    ...
    String latestTime = timeArray[lastIndex].as<String>();

    // Extract just the HH:MM timestamp from ISO 8601 string length variations
    if (latestTime.length() >= 16) {
      latestTime = latestTime.substring(11, 16);
    }

    // Space Weather NOAA Scale Mapping Logic
    ...
    char spaceBuf[50];
    snprintf(spaceBuf, sizeof(spaceBuf), "SPACE WX: Kp=%.1f (%s) UTC:%s", latestKp, stormStatus, latestTime.c_str());

    Serial.printf("[SPACE WX] Injecting MAVLink payload: %s\n", spaceBuf);
    sendMavlinkStatusText(spaceBuf);

    // Emit critical alert if geomagnetic activity threatens GNSS positioning lock or RTK integrity
    if (latestKp >= 5.0f) {
      vTaskDelay(pdMS_TO_TICKS(500));
...
void fetchSatellitesOverhead() {
  ...
  String requestURL = "https://api.n2yo.com/rest/v1/satellite/above/" + String(gps.lat, 5) + "/" + String(gps.lon, 5) + "/" + String(gps.alt, 2) + "/" + String((int)sat_rad_deg) + "/" + String((int)sat_cat) + "/&apiKey=" + N2YO_API_KEY;
...
void fetchAndSendToMAVLink() {
  ...
  String apiUrl = "https://opendata.adsb.fi/api/v3/lat/" + String(gps.lat, 6) + "/lon/" + String(gps.lon, 6) + "/dist/" + String((int)adsb_radius);
...
void send_mavlink_adsb(uint32_t icao, double lat, double lon, int32_t alt_ft, float heading_deg, String callsignStr, uint8_t emitter_type) {
  ...
  // Pack directed to MAV_COMP_ID_SHARED_OUT (156)
...
void handleGpsFixAcquisition(const char* source) {
  if (!gps_ready) {
    ...
    // --- DUAL IMMEDIATE WEATHER TRIGGER ---
    if (wx_enable > 0.5f) {
        ...
        case MAVLINK_MSG_ID_PARAM_REQUEST_LIST:
          {
            ...
            if (req_list.target_system == MAV_SYS_ID && req_list.target_component == MAV_COMP_ID_SHARED_OUT) {  // Changed to SHARED
              for (uint8_t i = 0; i < ONBOARD_PARAM_COUNT; i++) {
        ...
        case MAVLINK_MSG_ID_PARAM_REQUEST_READ:
          {
            ...
            if (read_req.target_system == MAV_SYS_ID && read_req.target_component == MAV_COMP_ID_SHARED_OUT) {  // Changed to SHARED
        ...
        case MAVLINK_MSG_ID_PARAM_SET:
          {
            ...
            if (set_param.target_system == MAV_SYS_ID && set_param.target_component == MAV_COMP_ID_SHARED_OUT) {  // Changed to SHARED
              char nameBuf[17] = { 0 };
